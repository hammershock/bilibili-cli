...
@click.option("--json", "as_json", is_flag=True)
def subtitle(bvid: str, cid: Optional[int], lang: Optional[str],
             limit: int, offset: int, show_all: bool, as_json: bool):
    """Show subtitle content for a video."""
    from bilicli.api.video import list_subtitles, get_subtitle_content
    client = get_client()
    try:
        tracks = list_subtitles(client, bvid, cid=cid)
    except BiliError as e:
        _err(str(e))
    if not tracks:
...
@click.option("--cid", type=int, default=None)
@click.option("--json", "as_json", is_flag=True)
def subtitle_langs(bvid: str, cid: Optional[int], as_json: bool):
    """List available subtitle languages for a video."""
    from bilicli.api.video import list_subtitles
    client = get_client()
    try:
        tracks = list_subtitles(client, bvid, cid=cid)
    except BiliError as e:
...
def danmaku(bvid: str, cid: Optional[int], as_json: bool, limit: int, offset: int, show_all: bool):
    """Fetch danmaku (bullet comments) for a video."""
    from bilicli.api.video import get_video_info, get_danmaku
    client = get_client()
    try:
        info = get_video_info(client, bvid)
        all_items = get_danmaku(client, bvid, cid=cid)
...
def comments(bvid: str, limit: int, offset: int, show_all: bool, sort: str, detail: bool, width: int, as_json: bool):
    ...
    client = get_client()
    # Fetch enough comments to cover offset + limit
    page_size = 20
    all_replies = []
    page = 1
    total_count = 0
...
def download(bvid: str, quality: str, output: str, cid: Optional[int], quiet: bool):
    """Download a video (requires ffmpeg)."""
    from bilicli.download import download_video
    client = get_client()
    qn = _QUALITY_CHOICES[quality]
    try:
        download_video(client, bvid, quality=qn, output_dir=output, cid=cid, quiet=quiet)
    except BiliError as e:
        _err(str(e))
...
def cover(bvid: str, output: str, quiet: bool):
